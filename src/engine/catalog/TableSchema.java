...
/**
 * Holds the complete schema for one table: its name, tableId, columns,
 * and a running count of how many pages it currently occupies on disk.
 *
 * Column byte offsets are computed automatically at construction time
 * by walking the column list in order — no manual offset tracking needed.
 *
 * pageCount is mutable so the heap file manager can increment it as
 * new pages are allocated, without needing to touch the catalog.
 */
public class TableSchema {
    ...
    public void incrementPageCount()         { this.pageCount++; }

    @Override
    public String toString() {
        return "TableSchema{id=" + tableId + ", name=" + tableName
            + ", recordSize=" + recordSize + "B, pages=" + pageCount
            + ", columns=" + columns + "}";
    }
}